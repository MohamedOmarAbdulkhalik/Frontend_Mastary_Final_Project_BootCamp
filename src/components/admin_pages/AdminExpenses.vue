...
<AdminLayout>
  <div class="container mx-auto px-4 py-10">
    ...
    <!-- ✅ اختيار المشروع -->
    <div class="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-lg max-w-lg mx-auto mb-8">
    ...
    <!-- ✅ عند اختيار مشروع -->
    <div v-if="selectedProject" class="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-lg max-w-3xl mx-auto">
      <h3 class="text-2xl font-bold mb-4 text-gray-900 dark:text-white text-center">
        Expenses for "{{ selectedProject.name }}"
      </h3>

      <p class="text-center text-sm mb-4 text-gray-700 dark:text-gray-300">
        <strong>Funds Received:</strong> ${{ selectedProject.fundsReceived }} |
        <strong>Current Expenses:</strong> ${{ selectedProject.expenses || 0 }} |
        <strong>Remaining:</strong> ${{ selectedProject.fundsReceived - (selectedProject.expenses || 0) }}
      </p>
      ...
      <table class="w-full border border-gray-300 mb-5">
        ...
        <tbody>
          <tr v-for="exp in getProjectExpenses(selectedProject.id)" :key="exp.id">
          ...
          <tr v-if="getProjectExpenses(selectedProject.id).length === 0">
            <td colspan="3" class="text-center text-gray-500 p-3">No expenses yet.</td>
          </tr>
        </tbody>
      </table>
